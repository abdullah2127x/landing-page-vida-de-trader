...
export default function Training() {
  return (
    <section
      id="training"
      className="relative py-24 bg-section-gradient overflow-hidden"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <PrimaryHeading
          title={
            <>
              CONTEÚDOS{" "}
              <span className="text-gold font-bold">DO TREINAMENTO</span>
            </>
          }
        />
        ...
        >
          {trainingModules.map((module) => (
            <SwiperSlide
              key={module.id}
              className="w-[280px]! sm:w-[320px]! transition-all duration-300"
            >
              <div className="group relative w-full h-[320px] sm:h-[400px] bg-black/50 rounded-lg overflow-hidden">
                ...
                <div className="absolute inset-0 z-0">
                  <Image
                    ...
                    alt={module.title}
                    fill
                    className="object-cover opacity-80 group-[.swiper-slide-active]:opacity-90 transition-opacity duration-300"
                  />
                  {/* Gradient overlay to ensure text is readable and match the dark aesthetic */}
                  <div className="absolute inset-0 bg-linear-to-b from-transparent  to-blue-dark" />
                </div>

                {/* Card Content */}
                <div className="absolute inset-0 z-10 p-6 flex flex-col justify-end text-center group-[.swiper-slide-active]:opacity-100 transition-opacity duration-300">
                  {module.subtitle && (
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-white-muted mb-1">
                      {module.subtitle}
                    </p>
                  )}
